...
from .ledger import append_predictions


def prepare_market(prices_path: str, fundamentals_path: str | None = None,
                   universe_snapshots_path: str | None = None) -> pd.DataFrame:
    prices = pd.read_csv(prices_path, parse_dates=["date"])
    if universe_snapshots_path and Path(universe_snapshots_path).exists():
        from .data import apply_universe_snapshots
        snapshots = pd.read_csv(universe_snapshots_path, parse_dates=["as_of"])
        prices = apply_universe_snapshots(prices, snapshots)
...
def predict_daily(prices_path: str, model_dir: str, prediction_date: str,
                  ...
                  universe_snapshots_path: str | None = None,
                  prediction_store: str | None = None) -> pd.DataFrame:
    data = prepare_market(prices_path, fundamentals_path, universe_snapshots_path)
    ...
    risk_columns = ["symbol", "close", "atr_pct_14", "volatility_20", "downside_volatility_20", "volume_ratio_20", "sector", *regime_columns]
    ...
    pred["market_regime"] = pred.apply(regime_label, axis=1) if all(c in pred.columns for c in regime_columns) else "unknown"
    pred["forecast_horizon"] = "next_session"
    pred["model_version"] = model_version
    result = final_trade_decision(pred, risk_config)
    if prediction_store:
        append_predictions(prediction_store, result)
    return result
