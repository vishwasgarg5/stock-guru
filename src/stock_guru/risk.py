...
def _effective_configs(predictions: pd.DataFrame, config: RiskConfig) -> pd.Series:
    """Return one effective RiskConfig per prediction row's market regime."""
...
def apply_risk_filters(predictions: pd.DataFrame, config: RiskConfig | None = None) -> pd.DataFrame:
    """Filter predictions using regime-aware volatility, confidence and return thresholds."""
    ...
    checks["confidence_ok"] = [
        _safe_float(out.get("rank_confidence", pd.Series(0.0, index=out.index)).loc[idx], 0.0) >= effective.loc[idx].min_confidence
        for idx in out.index
    ]
    checks["atr_ok"] = [
        _safe_float(out.get("atr_pct_14", pd.Series(float("nan"), index=out.index)).loc[idx]) <= effective.loc[idx].max_atr_pct
        for idx in out.index
    ]
...
def size_positions(predictions: pd.DataFrame, config: RiskConfig | None = None) -> pd.DataFrame:
    ...
    per_position_cap = pd.Series(
        [effective.loc[idx].max_position_pct for idx in raw.index], index=raw.index, dtype=float
    )
...
def final_trade_decision(predictions: pd.DataFrame, config: RiskConfig | None = None) -> pd.DataFrame:
    """Produce final trade/no-trade decisions after all regime-aware risk controls."""
    cfg = config or RiskConfig()
